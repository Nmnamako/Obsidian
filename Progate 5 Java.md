...
```
上記の様に、Vehicle型のインスタンスを受け取る様にすると
Carクラス、Bicycleクラスのインスタンスを受け取ることが可能

なぜ、carメソッドにVehicle型のクラス指定ができのか。
Carクラスは、Vehicleクラスを継承しているので、Carクラスの
インスタンスはCar型である前にVehicle型でもある。
Vehicle.java
``` java
abstract class Vehicle {
 Vehicle(String name, String color) {
...
```
上記の記述で継承されているので、データ型にVehicle型を入れることができる。
